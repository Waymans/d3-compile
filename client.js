...
const arr = [ 'Welcome','to a data-driven','Compilation','Welcome to a D3 Compilation' ];
...
// Get the data
const filenames = [ "dataM.json", "dataM_alt.json" ];
const dataset= [ 
  "dataM.json",
...
// Define the axes
const line_xAxis = d3.axisBottom(line_x).ticks(5);
const line_yAxis = d3.axisLeft(line_y).ticks(4);
// Define the line
const line_valueline = d3.line()
...
function updateLine() {
  ...
  d3.json(json, function(error, data) {
    ...
    let line_svg = d3.select("#lineGraph").transition();
...
function makeBar(data){
  const bar_margin = {top: 30, right: 20, bottom: 30, left: 50},
...
function makeDot(data){
  const html = data,
        m = {top: 20, right: 20, bottom: 0, left: 20},
        h = 550,
        w = 950,
        padding = 50;
...
function makeMap(jsonE, jsonC){
  ...
  const legend = svg.selectAll('.legend')
        .data(colors.ticks(6))
        .enter().append('g')
        .attr('transform', (d,i) => {return "translate(" + (w/1.6 + 40*i) + "," + "20)"})
...
const tree_m = {top: 20, right: 50, bottom: 20, left: 20},
      tree_h = 500 - tree_m.top - tree_m.bottom,
      tree_w = 950 - tree_m.right - tree_m.left;            
...
function updateTree() {
  ...
  d3.json(json, function(error, data) {
    ...
    const g = d3.select('.tree-svg').selectAll('g')
        .data(rootNode.leaves());
    ...
    /* legend */
    const leg = d3.select('.tree_legend')
