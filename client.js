...
var arr = [ 'Welcome','to a','data-driven','compilation','Welcome to a data-driven compilation' ];
var alphabet = "a data-driven compilation".split("");
var svg = d3.select("svg"),
    width = +svg.attr("width"),
    height = +svg.attr("height"),
    g = svg.append("g").attr("transform", "translate(-20,50)");
...
/* **************************************************** LINE ***************************************************** */
  ...
  var line_margin = {top: 30, right: 20, bottom: 30, left: 50},
      line_width = w/1.3 - line_margin.left - line_margin.right,
      line_height = h/1.3 - line_margin.top - line_margin.bottom;
  // Parse the date / time
  ...
  const html = data.monthlyVariance;
    ...
    const legend = svg.selectAll("legend")
                ...
                legend.append("text")
                      ...
                      .attr("dy", "18px")
                      .style("fill", (d, i)=> color(-d))
                      .style('text-shadow','2px 0px 5px black')
                      .text( (d, i)=> {
                        return labels[i]
                      });
}
